...
#             st.markdown(f"**✅ Based on uploaded data:**\n\n{answer}")
#         else:
#             answer = "🤖 I couldn't find a good match in the uploaded data."

#         # Save for possible Gemini fallback
...
#     st.markdown("---")
#     if st.button("🌐 Not satisfied? Search the web"):
#         with st.chat_message("assistant"):
#             st.markdown("🔍 Searching the web using Gemini...")
#             web_answer = answer_with_gemini(st.session_state["last_question"])
#             st.markdown(f"**🌐 Gemini Web Answer:**\n\n{web_answer}")
#         st.session_state.chat_history.append({"role": "assistant", "content": web_answer})
